    ...
    def __init__(self,
        ...
        if counter in transformer_blocks:
            if mixer_type == "attn":
                ...
                from pom import PoM  # lazy import: pom need not be on PYTHONPATH when mixer_type="attn"
                # PoM self-mixes window tokens [B_windows, N, C] (xc=None), a linear-time drop-in for the windowed attention it replaces.
                self.mixer = PoM(
                    dim=dim,
                    degree=2,
                    expand=2,
                    n_groups=1,
                    n_sel_heads=num_heads,
                    bias=qkv_bias,
                )
            else:
